...
function generate_graph_paper(array $p): \GdImage
{
        ...
        if ($sheetCol > 0) {
            if ($tabStyle === 'tape') {
                ...
                foreach (gp_seam_tabs($sTop, $gridBottom, $gridPx, $boxPx) as [$gt, $tt, $tb, $gb]) {
                    if ($tt > $gt) {
                        imagefilledrectangle($img, $marginPx, $gt, $marginPx + $tabPx, $tt, $cTabFill);
                    }

                    if ($gb > $tb) {
                        imagefilledrectangle($img, $marginPx, $tb, $marginPx + $tabPx, $gb, $cTabFill);
                    }
                }
            }
        }
        ...
        if ($sheetRow > 0) {
            if ($tabStyle === 'tape') {
                ...
                foreach (gp_seam_tabs($sLeft, $widthPx - $marginPx, $gridPx, $boxPx) as [$gl, $tl, $tr, $gr]) {
                    if ($tl > $gl) {
                        imagefilledrectangle($img, $gl, $marginPx, $tl, $marginPx + $tabPx, $cTabFill);
                    }

                    if ($gr > $tr) {
                        imagefilledrectangle($img, $tr, $marginPx, $gr, $marginPx + $tabPx, $cTabFill);
                    }
                }
            }
        }
    ...
    for ($x = $gridX0; $x <= $widthPx - $marginPx; $x += $gridPx) {
        ...
        if (intdiv($absX, $gridPx) % $gridPerBox === 0) {
            gp_text(
                $img,
                gp_index_label(intdiv($absX, $gridPx)),
                $x + (int) ($gridPx / 2),
                $labelCy,
                $cLabel,
                $labelPt,
    ...
    if ($sheetCol === 0 && $sheetRow === 0) {
        ...
        $activeLines = array_values(array_filter($titleLines, fn($l): bool => trim((string) $l) !== ''));
    ...
    if ($dungeonCols && $dungeonRows) {
        ...
        $hline = function (int $y, int $x0, int $x1) use ($img, $cBorder, $sx0, $sx1): void {
            $x0 = max($x0, $sx0);
            $x1 = min($x1, $sx1);
            if ($x0 < $x1) {
                imageline($img, $x0, $y, $x1, $y, $cBorder);
            }
        };
        $vline = function (int $x, int $y0, int $y1) use ($img, $cBorder, $sy0, $sy1): void {
            $y0 = max($y0, $sy0);
            $y1 = min($y1, $sy1);
            if ($y0 < $y1) {
                imageline($img, $x, $y0, $x, $y1, $cBorder);
            }
        };
            ...
            if ($ly >= 0) {
                for ($x = $lx; $x < $rx; $x += $gridPx) {
                    ...
                    if ($mid >= $sx0 && $mid <= $sx1) {
                        gp_text(
                            $img,
                            gp_index_label(intdiv($x - $lx, $gridPx)),
                            $mid,
                            $ly,
                            $cDLabel,
                            $labelPt,
    ...
    if ($startCol !== null && $startRow !== null) {
        ...
        if (
            $localSx >= $marginPx && $localSx < $widthPx - $marginPx
            ...
            gp_text(
                $img,
                'S',
                $localSx + (int) ($gridPx / 2),
                $localSy + (int) ($gridPx / 2),
                gp_color($img, [180, 120, 0]),
                $ptSize,
                $font
            );
        }
